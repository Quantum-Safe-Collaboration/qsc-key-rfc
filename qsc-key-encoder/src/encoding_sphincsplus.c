...
#define SPHINCSPLUSXXX_CRYPTO_ASN1_PUBLICKEYBYTES(NAME) \
  ASN_TLLEN(SPHINCSPLUS_##NAME##_CRYPTO_PUBLICKEYBYTES) + \
  ASN_TLLEN(SPHINCSPLUS_##NAME##_PKSEED) + SPHINCSPLUS_##NAME##_PKSEED + \
  ASN_TLLEN(SPHINCSPLUS_##NAME##_PKROOT) + SPHINCSPLUS_##NAME##_PKROOT
...
#define SPHINCSPLUSXXX_CRYPTO_ASN1_SUBJECTPUBLICKEYINFOBYTES(NAME, NAME2) \
  ASN_TLLEN(SPHINCSPLUSXXX_CRYPTO_ASN1_PUBLICKEYBYTES(NAME)) + \
  ASN_TLLEN(ASN_OIDLEN(QSC_ALGORITHM_SIG_SPHINCSPLUS_##NAME2##_OID)) + \
  ASN_OIDLEN(QSC_ALGORITHM_SIG_SPHINCSPLUS_##NAME2##_OID) + \
  ASN_TLLEN(0) + \
  ASN_TLLEN(SPHINCSPLUSXXX_CRYPTO_ASN1_PUBLICKEYBYTES(NAME)) + SPHINCSPLUSXXX_CRYPTO_ASN1_PUBLICKEYBYTES(NAME)
...
#define SPHINCSPLUSXXX_CRYPTO_ASN1_SECRETKEYBYTES(NAME) \
  ASN_TLLEN(SPHINCSPLUS_##NAME##_CRYPTO_SECRETKEYBYTES) + \
  ASN_TLLEN(1) + 1 + \
  ASN_TLLEN(SPHINCSPLUS_##NAME##_SKSEED) + SPHINCSPLUS_##NAME##_SKSEED + \
  ASN_TLLEN(SPHINCSPLUS_##NAME##_SKPRF) + SPHINCSPLUS_##NAME##_SKPRF + \
...
#define SPHINCSPLUSXXX_CRYPTO_ASN1_PRIVATEKEYINFOBYTES(NAME, NAME2) \
  ASN_TLLEN(ASN_TLLEN(1) + 1 + ASN_TLLEN(ASN_OIDLEN(QSC_ALGORITHM_SIG_SPHINCSPLUS_##NAME2##_OID)) + ASN_OIDLEN(QSC_ALGORITHM_SIG_SPHINCSPLUS_##NAME2##_OID) + ASN_TLLEN(0) + ASN_TLLEN(SPHINCSPLUSXXX_CRYPTO_ASN1_SECRETKEYBYTES(NAME)) + SPHINCSPLUSXXX_CRYPTO_ASN1_SECRETKEYBYTES(NAME)) + \
  ASN_TLLEN(1) + 1 + \
  ASN_TLLEN(ASN_OIDLEN(QSC_ALGORITHM_SIG_SPHINCSPLUS_##NAME2##_OID)) + \
  ASN_OIDLEN(QSC_ALGORITHM_SIG_SPHINCSPLUS_##NAME2##_OID) + \
  ASN_TLLEN(0) + \
  ASN_TLLEN(SPHINCSPLUSXXX_CRYPTO_ASN1_SECRETKEYBYTES(NAME)) + SPHINCSPLUSXXX_CRYPTO_ASN1_SECRETKEYBYTES(NAME)

#define SPHINCSPLUSXXX_CRYPTO_ASN1_PRIVATEKEYINFOBYTES_NOOPTIONAL(NAME, NAME2) \
  ASN_TLLEN(ASN_TLLEN(1) + 1 + ASN_TLLEN(ASN_OIDLEN(QSC_ALGORITHM_SIG_SPHINCSPLUS_##NAME2##_OID)) + ASN_OIDLEN(QSC_ALGORITHM_SIG_SPHINCSPLUS_##NAME2##_OID) + ASN_TLLEN(0) + ASN_TLLEN(SPHINCSPLUSXXX_CRYPTO_ASN1_SECRETKEYBYTES_NOOPTIONAL(NAME)) + SPHINCSPLUSXXX_CRYPTO_ASN1_SECRETKEYBYTES_NOOPTIONAL(NAME)) + \
  ASN_TLLEN(1) + 1 + \
  ASN_TLLEN(ASN_OIDLEN(QSC_ALGORITHM_SIG_SPHINCSPLUS_##NAME2##_OID)) + \
  ASN_OIDLEN(QSC_ALGORITHM_SIG_SPHINCSPLUS_##NAME2##_OID) + \
  ASN_TLLEN(0) + \
  ASN_TLLEN(SPHINCSPLUSXXX_CRYPTO_ASN1_SECRETKEYBYTES_NOOPTIONAL(NAME)) + SPHINCSPLUSXXX_CRYPTO_ASN1_SECRETKEYBYTES_NOOPTIONAL(NAME)
...
#define SPHINCSPLUS_ASNTLP_PK(NAME) \
  static const qsc_asntl_t SPHINCSPLUS_##NAME##_asntlp_pk[] = { \
      { \
          ...
          .asnlen = SPHINCSPLUSXXX_CRYPTO_ASN1_PUBLICKEYBYTES(NAME) - ASN_TLLEN(SPHINCSPLUSXXX_CRYPTO_ASN1_PUBLICKEYBYTES(NAME)), \
          .asnenc_flag = 1, \
      }, \
      { \
...
#define SPHINCSPLUS_ASNTLP_SK(NAME) \
  static const qsc_asntl_t SPHINCSPLUS_##NAME##_asntlp_sk[] = { \
      { \
          .asntag = ASN1_SEQUENCE, \
          .asnlen = SPHINCSPLUSXXX_CRYPTO_ASN1_SECRETKEYBYTES(NAME) - ASN_TLLEN(SPHINCSPLUSXXX_CRYPTO_ASN1_SECRETKEYBYTES(NAME)), \
          .asnenc_flag = 1, \
      }, \
      { \
          .asntag = ASN1_INT, \
